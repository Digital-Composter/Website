...
export default function SimpleLineChart() {
  ...
  const indicatorData = [
    ...
    { title: 'Air Temperature', value: tempAmbiance, unit: '°C' },
    { title: 'Air Humidity', value: hambiance, unit: '%' },
    { title: 'Phase', value: phase, unit: '' },
  ];

  const IndicatorBox = ({ title, value, unit }) => (
    <div className="rounded-lg w-44 h-24 flex flex-col justify-center items-center">
      <h1 className='text-center text-sm md:text-lg'>{title}</h1>
      <div className="flex items-baseline">
        <p className="text-center font-semibold text-2xl md:text-2xl text-black">{value}</p>
        <span className="text-center text-sm md:text-base text-green ml-1">{unit}</span>
      </div>
    </div>
  );
    ...
    <section className='relative flex flex-col h-screen -mt-2 ' id='dashboard'>
      ...
      <div className='flex flex-wrap justify-center gap-4 mt-4'>
        {indicatorData.map((indicator, index) => (
          <Card key={index} className="w-44 h-24 flex justify-center items-center">
      ...
      <div className="flex flex-col lg:flex-row w-full justify-center items-center">
        <Card className="w-full lg:mx-40 sm:mx-4 sm:my-8">
